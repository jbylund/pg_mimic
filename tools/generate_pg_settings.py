...
def _entry_for(row: dict) -> dict:
    """One parameter, with everything that does not apply to it left out.

    `default`, `vartype` and `context` are always present: every parameter has all
    three. The rest constrain what a *value* may be, and most parameters constrain
    nothing -- 320 of 398 have no unit, 357 have no enum -- so carrying the empties
    would triple the file to say nothing. A reader takes an absent key as "no such
    constraint", which is what `settings_catalog` does.
    """
    entry = {
    ...
    # `unit` is the bucket: a parameter carrying one takes `8MB` or a bare number in
    # that unit, and reads back in whichever unit divides evenly.
    if row["unit"]:
        entry["unit"] = row["unit"]
    if row["enumvals"]:
        entry["enumvals"] = list(row["enumvals"])
    if row["min_val"]:
        entry["min_val"] = row["min_val"]
    if row["max_val"]:
        entry["max_val"] = row["max_val"]
    if row["short_desc"]:
...
def _render_document(document: dict) -> str:
    """One line per parameter, rather than json.dumps's one line per *field*.

    A regenerated file then diffs as the parameters that changed, instead of as
    several thousand lines of re-indented punctuation.
    """
    ...
    last = len(settings) - 1
    for index, (name, entry) in enumerate(settings.items()):
        comma = "" if index == last else ","
        lines.append(f"  {json.dumps(name)}: {json.dumps(entry, separators=(', ', ': '))}{comma}")
    lines += [" }", "}"]
    return "\n".join(lines) + "\n"
...
def main(dsn: str) -> None:
    ...
    settings = {row["name"].lower(): _entry_for(row) for row in rows}
    document = {
