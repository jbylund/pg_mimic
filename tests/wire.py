...
def make_startup(user: str = "test", database: str = "test") -> bytes:
    params = _cstring("user") + _cstring(user) + _cstring("database") + _cstring(database) + b"\x00"
    body = pack_int32(196608) + params
...
def make_bind(statement_name: str = "", portal_name: str = "") -> bytes:
    payload = _cstring(portal_name) + _cstring(statement_name) + pack_int16(0) + pack_int16(0) + pack_int16(0)
...
async def connect_and_get_backend_key(port: int) -> tuple[asyncio.StreamReader, asyncio.StreamWriter, int, int]:
